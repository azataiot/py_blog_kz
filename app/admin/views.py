...
def editArticles(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(article)
        db.session.commit()
        flash(u'Боуэн сәтті жаңартылды!', 'success')
...
def manage_articles():
    ...
    types = [(t.id, t.name) for t in ArticleType.query.all()]
    types.append((-1, u'Барлық санаттар'))
    form.types.choices = types
    sources = [(s.id, s.name) for s in Source.query.all()]
    sources.append((-1, u'Барлық көздер'))
    form.source.choices = sources
...
def delete_article():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            flash(u'Жою сәтсіз аяқталды!', 'danger')
        else:
            flash(u'Bowen және%s ескертулерін сәтті жойды!' % count, 'success')
    if form.errors:
        flash(u'Жою сәтсіз аяқталды!', 'danger')
...
def delete_articles():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash(u'% S хабарламаларын және% s шолуларын сәтті жойды!' % (len(articleIds), count), 'success')
    if form.errors:
        flash(u'Жою сәтсіз аяқталды!', 'danger')
...
def disable_comment(id):
    ...
    db.session.commit()
    flash(u'Түсініктемені сәтті орындаңыз!', 'success')
...
def enable_comment(id):
    ...
    db.session.add(comment)
    db.session.commit()
    flash(u'Резюме коментарийлердің сәттілігін көрсету!', 'success')
...
def delete_comment(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(u'Пікірді жою сәтсіз аяқталды!', 'danger')
    else:
        flash(u'Түсініктемелерді жою!', 'success')
...
def manage_comments():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(comment)
        db.session.commit()
        flash(u'Пікірді сәтті жіберіңіз!', 'success')
        return redirect(url_for('.manage_comments'))
    if form.errors:
        flash(u'Пікір жіберу сәтсіз аяқталды! Қателер бар-жоқтығын тексеріңіз.', 'danger')
        return redirect(url_for('.manage_comments'))
...
def delete_comments():
    ...
    if form2.validate_on_submit():
        ...
        try:
            ...
            flash(u'%S пікірін сәтті жойды!' % count , 'success')
    if form2.errors:
        flash(u'Жою сәтсіз аяқталды!', 'danger')
...
def manage_articleTypes():
    ...
    if form.validate_on_submit():
        ...
        if articleType:
            flash(u'Санат қосылмады! Санат атауы бұрыннан бар.', 'danger')
        else:
            ...
            db.session.commit()
            flash(u'Санат табысын қосыңыз!', 'success')
        return redirect(url_for('.manage_articleTypes'))
    if form.errors:
        flash(u'Санат қосылмады! Қателер бар-жоқтығын тексеріңіз.', 'danger')
        return redirect(url_for('.manage_articleTypes'))
...
# Тексеру, тексеру формасын жіктеуді жоғарыда келтірілген түрде қосу, ұсыныс бөлек жазылуы мүмкін, мұнда тек әдісті ұсыну керек, біріншісі ақиқат
# Кішкене кодты бөлек жазып алса да, бұл логика айқынырақ болады
#Айта кету керек, тағы бір нәрсе - бұл екі көзқараста бірдей көзқарастарды жазу арқылы қиындықтар туындағандықтан, оларды бөлек жазу ұсынылады.
    ...
    if form2.validate_on_submit():
        ...
        if articleType.is_protected:
            if form2.name.data != articleType.name or \
                ...
                flash(u'Сіз жүйенің әдепкі классификациясы мен оның шарлауының төлсипаттарын ғана өзгерте аласыз!', 'danger')
            else:
                ...
                db.session.add(articleType)
                db.session.commit()
                flash(u'Жүйенің әдепкі классификациясының сәттілігін өзгерту!', 'success')
            ...
            and ArticleType.query.filter_by(name=form2.name.data).first().id != articleType_id:
                flash(u'Жіктеуді өзгерту сәтсіз аяқталды! Санат атауы бұрыннан бар.', 'danger')
        else:
            ...
            db.session.add(articleType)
            db.session.commit()
            flash(u'Өзгертілген классификацияның жетістігі!', 'success')
        return redirect(url_for('.manage_articleTypes', page=page))
    if form2.errors:
        flash(u'Жіктеуді өзгерту сәтсіз аяқталды! Қателер бар-жоқтығын тексеріңіз.', 'danger')
        return redirect(url_for('.manage_articleTypes', page=page))
...
def delete_articleType(id):
    ...
    if articleType.is_protected:
        flash(u'Ескерту: Сізде жүйенің әдепкі санатын жоюға рұқсатыңыз жоқ!', 'danger')
    ...
    try:
        ...
        db.session.rollback()
        flash(u'Санатты жою сәтсіз аяқталды!', 'danger')
    else:
        flash(u'Жіктеудің сәттілігін жою! Сонымен қатар,%s хабарлардың% <жіктелмеген> деп қосыңыз.' % count, 'success')
    return redirect(url_for('admin.manage_articleTypes', page=page))
...
def manage_articleTypes_nav():
    ...
    if form.validate_on_submit():
        ...
        if menu:
            page = page
            flash(u'Навигация қосу сәтсіз аяқталды! Шарлау атауы бұрыннан бар.', 'danger')
        else:
            ...
            db.session.commit()
            page = -1
            flash(u'Навигациялық табысты қосыңыз!', 'success')
    ...
    if form2.validate_on_submit():
        ...
        if Menu.query.filter_by(name=name).first() \
            and Menu.query.filter_by(name=name).first().id != nav_id:
                flash(u'Навигацияны өзгерту сәтсіз аяқталды! Шарлау атауы бұрыннан бар.', 'danger')
        else:
            nav = Menu.query.get_or_404(nav_id)
            nav.name = name
            db.session.add(nav)
            db.session.commit()
            flash(u'Навигация жетістігін өзгерту!', 'success')
        return redirect(url_for('admin.manage_articleTypes_nav', page=page))
    if form2.errors:
        flash(u'Навигацияны өзгерту сәтсіз аяқталды! Қателер бар-жоқтығын тексеріңіз.', 'danger')
        return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def delete_nav(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(u'Шарлауды жою сәтсіз аяқталды!', 'danger')
    else:
        flash(u'Шарлауды сәтті жою! Сонымен бірге% s бастапқы навигациясының навигациясы ешбірге орнатылмаған.' % count, 'success')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def nav_sort_up(id):
    ...
    if pre_menu:
        ...
        db.session.commit()
        flash(u'Навигация сәтті өтті!', 'success')
    else:
        flash(u'Бұл шарлау алдыңғы жақта!', 'danger')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def nav_sort_down(id):
    ...
    if latter_menu:
        ...
        flash(u'Навигация аяқталды!', 'danger')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def custom_blog_info():
    ...
    navbars = [(1, u'Шебер қара'), (2, u'Ақылды ақ')]
    ...
    if form.validate_on_submit():
        ...
        flash(u'Блогтың негізгі ақпаратын сәтті өзгертіңіз!', 'success')
        return redirect(url_for('admin.custom_blog_info'))
...
def delete_plugin(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(u'Плагиндерді жою сәтсіз аяқталды!', 'danger')
    else:
        flash(u'Плагинді сәтті жою!' ,'success')
...
def plugin_sort_up(id):
    ...
    if pre_plugin:
        ...
        db.session.commit()
        flash(u'Қосылатын модульді сәтті көтерді!', 'success')
    else:
        flash(u'Плагин алдыңғы жағында!', 'danger')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def plugin_sort_down(id):
    ...
    if latter_plugin:
        ...
        db.session.commit()
        flash(u'Плагинді сәтті түсірді!', 'success')
    else:
        flash(u'Плагин аяқталды!', 'danger')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def disable_plugin(id):
    ...
    db.session.add(plugin)
    db.session.commit()
    flash(u'Плагиннің табысын өшіру!', 'success')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def enable_plugin(id):
    ...
    db.session.add(plugin)
    db.session.commit()
    flash(u'Плагиннің жетістігін қосу!', 'success')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
    ...
    if form.validate_on_submit():
        ...
        if plugin:
            form = AddBlogPluginForm(title=title, note=form.note.data,
                                     content=form.content.data)
            flash(u'Плагиндерді қосу сәтсіз аяқталды! Плагин атауы бар.', 'danger')
            ...
            db.session.add(plugin)
            db.session.commit()
            flash(u'Плагиндерді сәтті қосыңыз!', 'success')
...
def edit_plugin(id):
    ...
    if form.validate_on_submit():
        ...
        if plugin_check and plugin_check.id != id:
            flash(u'Плагинді өзгерту сәтсіз аяқталды! Плагин атауы бар.', 'danger')
            ...
            db.session.add(plugin)
            db.session.commit()
            flash(u'Плагинді сәтті өзгертіңіз!', 'success')
        return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def change_password():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.old_password.data):
            ...
            db.session.commit()
            flash(u'Құпия сөз табысын өзгерту!', 'success')
            return redirect(url_for('admin.account'))
        else:
            flash(u'Құпия сөзді өзгерту сәтсіз аяқталды! Құпия сөз дұрыс емес!', 'danger')
            return redirect(url_for('admin.account'))
...
def edit_user_info():
    ...
    if form2.validate_on_submit():
        if current_user.verify_password(form2.password.data):
            ...
            db.session.add(current_user)
            db.session.commit()
            flash(u'Пайдаланушы туралы ақпаратты сәтті өзгертіңіз!', 'success')
            return redirect(url_for('admin.account'))
        else:
            flash(u'Пайдаланушы ақпаратын өзгерту сәтсіз аяқталды! Құпия сөз дұрыс емес!', 'danger')
            return redirect(url_for('admin.account'))
